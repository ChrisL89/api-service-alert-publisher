...
const sendObject = (dataObject, callback) => {
  kafkaClient.schemaRepository.getSchemaEntityByTopic(getTopic(dataObject), function (error, schemaEntity) {
    if (error) {
      log.error(error, 'api-service-alert-publisher:kafka-transfer:get-topic:error', dataObject.asJSON());
      callback(error);
      return;
    }
...
const getTopic = dataObject => {
  ...
  const event = new EventEntity(dataObject.eventType, dataObject.eventType, 'monitoring-alert');
  return event.topic;
};
